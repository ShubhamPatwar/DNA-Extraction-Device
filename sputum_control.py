...
def run_motor_sequence_sputum():
    ...
    try:
        ...
        mixer(2,1200)                  ### 1200-20 min LYSIS #######
        if common.stop_flag: return
        safe_sleep(0.5)

        if common.stop_flag: return
        pause_event.wait()
        motion_motor(850, 2, 0.0004, 'up')
        if common.stop_flag: return
        safe_sleep(0.5)

        if common.stop_flag: return           ####### BEAD COLLECTING #######
        pause_event.wait()
        ...
        if common.stop_flag: return
        pause_event.wait()
        motion_motor_both(1700, 1, 2, 0.0002, 'down')
        if common.stop_flag: return
        ...
        if common.stop_flag: return
        pause_event.wait()
        motion_motor_both(1700, 1, 2, 0.0002, 'up')
        if common.stop_flag: return
        safe_sleep(0.5)
        ...
        mixer(2,600)                  ### 600-10 min LYSIS WITH BEADS #######
        if common.stop_flag: return
        safe_sleep(0.5)
        ...
        if common.stop_flag: return
        pause_event.wait()
        motion_motor_both(300, 1, 2, 0.0002, 'down')
        if common.stop_flag: return
        timer_module.start_timer(30)
        safe_sleep(30)
        ...
        pause_event.wait()
        timer_module.start_timer(600)
        wash_mixer(2, 600)             ####### 10 MIN WASH-1
        if common.stop_flag: return
        safe_sleep(0.5)
        ...
        for i in range(2):
            ...
            if last_flag:
                ...
                if common.stop_flag: return             ##### ELUTION ######
                pause_event.wait()
                ...
                last_mixer(2, 300)        ####### ELUTION 5-MIN 
                if common.stop_flag: return
                safe_sleep(0.5)
                ...
                pause_event.wait()
                timer_module.start_timer(600)
                wash_mixer(2, 600)      ###### WASH-2, 10-MIN
                if common.stop_flag: return
                safe_sleep(0.5)
